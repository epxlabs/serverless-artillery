...
const constants = {
  CompatibleServerlessSemver: '>=3.0.0',
...
`,
   ...
   */
  findServicePath: (options = {}) => {
    const cwd = process.cwd()
    const localServerlessPath = path.join(cwd, 'serverless.yml')

    if (options.config && impl.fileExists(options.config)) {
      return path.dirname(options.config)
    } else if (impl.fileExists(localServerlessPath)) {
  ...
  serverlessRunner: (options) => {
    ...
    const { commands, options: slsOptions } = Serverless.resolveCliArgs()
    // proceed with using SLS
    const serviceDir = impl.findServicePath(options)
    const configuration = yaml.load(fs.readFileSync(`${serviceDir}/serverless.yml`).toString().replace(/\${opt:stage}/g, options.stage))
    fs.writeFileSync(`${serviceDir}/serverless.yml`, yaml.dump(configuration))
  ...
  */
  serverlessLoader: () => {
    ...
    const { commands, options } = Serverless.resolveCliArgs()
    const configuration = yaml.load(fs.readFileSync(`${serviceDir}/serverless.yml`).toString().replace(/\${opt:stage}/g, options.stage))
    fs.writeFileSync(`${serviceDir}/serverless.yml`, yaml.dump(configuration))
    const serverless = new Serverless({
      commands,
    ...
    return serverless.init()
    // TODO: Resolving the variables in lambda/serverless.yml is near impossible with Serverless v3.
    // The variable resolver is too deeply buried and requires too many internal arguments to be easily dealt with here
    // WORKAROUND: The serverless.yml file has been updated to only reference ${opt:stage} variables.
    // The ${opt:stage} variables are being manually replaced via a regex above.

      // .then(() => serverless.variables.populateService(serverless.pluginManager.cliOptions)) DO NOT USE
      .then(() => {
